...
export function MediaDropArea() {
  const [dragActive, setDragActive] = useState(false);
  const inputRef = useRef<HTMLInputElement | null>(null);
  const { addFiles } = useUploadMediaForm();
  ...
  return (
    <Container>
      ...
      >
        <button onClick={() => inputRef.current?.click()} data-testid="file-select-button">
          마우스로 파일을 끌어오거나 여기를 클릭하세요
          <Hint>이미지/동영상 파일을 여러 개 선택할 수 있어요</Hint>
        </button>
        <input
          ref={inputRef}
          type="file"
