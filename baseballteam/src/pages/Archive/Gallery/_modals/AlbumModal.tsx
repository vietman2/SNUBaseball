...
export function AlbumModal({ toggleModal }: Readonly<Props>) {
  const [title, setTitle] = useState<string>("");
  const [membersOnly, setMembersOnly] = useState<boolean>(false);

  const { createNewAlbum, editAlbum, selectedAlbum } = useAlbum();

  const handleSubmit = async () => {
    if (selectedAlbum) {
      const result = await editAlbum(selectedAlbum.id, title, membersOnly);
      if (result) {
        toggleModal();
      } else {
      ...
      const result = await createNewAlbum(title, membersOnly);
      if (result) {
        toggleModal();
      } else {
        window.alert("오류가 발생했습니다. 다시 시도해주세요.");
  ...
  useEffect(() => {
    if (selectedAlbum) {
      ...
      setMembersOnly(selectedAlbum.members_only);
    } else {
      setTitle("");
    ...
    <Overlay onClick={toggleModal}>
      <Modal onClick={(e) => e.stopPropagation()}>
        <Container>
          ...
          <button onClick={handleSubmit} data-testid="submit-new-album">
