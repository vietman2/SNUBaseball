...
class PasswordValidator:
    ...
    def validate(self, value, user=None):
        ...
        if not re.search(r"[A-Z]", value):
            raise SNUBaseballException("비밀번호는 최소 하나의 대문자를 포함해야 합니다.")
        if not re.search(r"[a-z]", value):
            raise SNUBaseballException("비밀번호는 최소 하나의 소문자를 포함해야 합니다.")
        if not re.search(r"\d", value):
            raise SNUBaseballException("비밀번호는 최소 하나의 숫자를 포함해야 합니다.")
        if not re.search(r"[!@#$%^&*(),.?\":{}|<>]", value):
            raise SNUBaseballException("비밀번호는 최소 하나의 특수문자를 포함해야 합니다.")
