...
class RegisterTestCase(SNUBaseballTestCase):
    def setUp(self):
    ...
    def test_register_student_id_mismatch(self):
        data = self.valid_data.copy()
        data["student_id"] = "2025-54321"
        response = self.client.post(self.url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data["message"], "학번이 일치하지 않습니다.")
    ...
    def test_register_invalid_data(self):
        ...
        response = self.client.post(self.url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("아이디를 입력해주세요.", response.data["message"])
